...
export class ProductsService implements IBaseProductService, IProductService {
  ...
  async createProduct(createProductDto: CreateProductDto): Promise<Product> {
    try {
      ...
      this.validationService.validateStock(
      ...
      this.logger.log(
        `Product created successfully: ${product.get('name')}`,
        'ProductsService',
      );
      return product;
    } catch (error) {
  ...
  async getAllProducts(): Promise<Product[]> {
    try {
      ...
      this.logger.log(
        `Retrieved ${products.length} active products`,
        'ProductsService',
      );
      return products;
    } catch (error) {
  ...
  async getProductById(id: string): Promise<Product> {
    try {
      ...
      this.logger.log(
        `Retrieved product: ${product.get('name')}`,
        'ProductsService',
      );
      return product;
    } catch (error) {
  ...
  async updateProduct(
    ...
    try {
      ...
      this.logger.log(
        `Product updated successfully: ${product.get('name')}`,
        'ProductsService',
      );
      return product;
    } catch (error) {
  ...
  async getProductsByIds(ids: string[]): Promise<Product[]> {
    try {
      // Validate all UUIDs
      ids.forEach((id) =>
        this.validationService.validateUUID(
          id,
      ...
      this.logger.log(
        `Retrieved ${products.length} products by IDs`,
        'ProductsService',
      );
      return products;
    } catch (error) {
  ...
  async findProductsByPriceRange(
    ...
    try {
      this.validationService.validatePrice(
        minPrice,
        'ProductsService.findProductsByPriceRange',
      );
      this.validationService.validatePrice(
        maxPrice,
        'ProductsService.findProductsByPriceRange',
      ...
      const products = await this.productRepository.findProductsByPriceRange(
        minPrice,
        maxPrice,
      );
      this.logger.log(
  ...
  async findLowStockProducts(threshold: number = 10): Promise<Product[]> {
    try {
      this.validationService.validateStock(
        threshold,
        'ProductsService.findLowStockProducts',
      );

      const products =
        await this.productRepository.findLowStockProducts(threshold);
      this.logger.log(
        `Found ${products.length} products with low stock (≤${threshold})`,
        'ProductsService',
      );
      return products;
    } catch (error) {
      this.logger.error(
        'Failed to find low stock products',
        error,
        'ProductsService.findLowStockProducts',
      );
      throw BoomExceptionFactory.badRequest(
  ...
  async updateStock(
    ...
    try {
      ...
      this.logger.log(
        `Stock updated for product ${productId}: ${product.stock} → ${newStock}`,
        'ProductsService',
      );
